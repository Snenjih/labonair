...
Package manager: **pnpm**. Bundle id: `com.labonair.app`.
...
- **Key storage**: OS keychain via `keyring`. Service/account constants in `config.ts` (`KEYRING_SERVICE = "labonair-ai"`). Never persist keys to disk, settings store, or `localStorage`.
