...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // The transfer worker reuses this same unified session registry
            // (per the russh migration's session-model decision) instead of a
            // second, separate SFTP-only map — cloned before `app.manage`
            // moves it, since `SshState` is just a cheap `Arc<Mutex<..>>` handle.
            ...
            // Constructed via `Default` before the worker is spawned and before
            // `app.manage` runs, so there's no window where a settings-sync
            // call could race the worker's own startup — the atomics already
            // hold sane defaults the moment the app can receive any IPC call.
            let transfer_settings = std::sync::Arc::new(TransferSettings::default());
            let transfer_settings_for_worker = transfer_settings.clone();
            tauri::async_runtime::spawn(async move {
                run_worker(rx, ssh_state_for_worker, app_handle, conflicts_for_worker, transfer_settings_for_worker).await;
            });
