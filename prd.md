...
**Project Name:** Labonair (Hard-fork of Terax)
...
**Labonair** is an evolution of the open-source "Terax" AI-terminal project. While Terax provides an incredible foundation for local terminal emulation, local file exploring, an integrated CodeMirror editor, and AI assistance, **Labonair transforms it into the ultimate Remote Workspace for Sysadmins and Backend Developers.**

Labonair introduces powerful, secure, and native remote capabilities (SSH and SFTP) without compromising the lightweight nature and 60-FPS performance of the base application. It directly competes with commercial tools like Termius and bloated Electron apps like Tabby, but completely open-source and natively integrated with macOS.

### 1.1 Out of Scope (Version 1.0 of Labonair)
...
Labonair inherits Terax's highly optimized, modular stack, and extends it with specific remote-networking crates.
...
- **Identity:** Rename all "Terax" references in `tauri.conf.json`, `package.json`, and the UI to "Labonair".
- **Identifier:** Change bundle identifier from `app.crynta.terax` to `com.labonair.app`.
...
- **Storage:** Host metadata (UUID, Name, IP, Port, Username, Auth Method, Group) is stored in a local SQLite database (`~/.local/share/com.labonair.app/labonair.db`).
...
- **Workflow:** 
  ...
  2. Rust downloads the file to `/tmp/labonair_edits/<uuid>_<filename>`.
...
- **Local File Explorer & AI Panel:** Ensure the existing Terax AI side-panel and Local Explorer sidebar (`src/modules/explorer`) remain fully functional and visually cohesive with the new Labonair tabs.
