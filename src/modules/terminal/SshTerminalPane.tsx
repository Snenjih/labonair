  ...
  function SshTerminalPane({ sessionId, session, isActive, tabVisible = true, onSearchReady}, ref) {
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        // Register all three event listeners first — before any async gap —
        // so SSH output that arrives during font-load is captured in
        // earlyBuffer rather than silently dropped. listen() in Tauri v2 is
        // async (IPC round-trip); without await the handler isn't registered
        // until after document.fonts.load(), which can take hundreds of ms.
        const unlistenOutput = await listen<{ session_id: string; data: string }>("ssh_pty_output", (event) => {
        ...
        const unlistenLost = await listen<{ session_id: string; reason: string }>("ssh_connection_lost", ({ payload }) => {
        ...
        if (disposed) { unlistenLost(); return; }
        cleanups.push(unlistenLost);

        const unlistenEstablished = await listen<{ session_id: string }>("session_established", ({ payload }) => {
          if (payload.session_id !== sessionId) return;
          reconnectAttemptRef.current = 0;
          setReconnectAttempt(0);
          setAutoReconnectFailed(false);
        });
        if (disposed) { unlistenEstablished(); return; }
        cleanups.push(unlistenEstablished);
        ...
        if (prefs.terminalUseWebGL) {
          ...
          const attachWebGL = () => {
            ...
            try {
              ...
              webgl.onContextLoss(() => {
                webgl.dispose();
                // GPU context was lost. xterm.js falls back to DOM renderer
                // automatically; re-attach WebGL after 1 s so the terminal
                // returns to GPU rendering.
