      ...
      <div className="flex items-start gap-2 px-2.5 py-1.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-1.5 font-mono text-[11.5px]">
            <span className="truncate text-foreground">{basename(item.path)}</span>
            {isNew ? <span className="text-[10px] text-success">new</span> : null}
          </div>
          <div className="truncate font-mono text-[10px] text-muted-foreground">{item.path}</div>
          {stats ? (
...
function UnifiedDiffPreview({ original, proposed }: { original: string; proposed: string }) {
  ...
  if (lines.length === 0) {
    return <div className="text-[11px] italic text-muted-foreground">no line-level changes</div>;
  }
      ...
      <div className="max-h-72 overflow-auto">
        {shown.map((l, i) => (
          <div
            ...
            className={cn(
              "flex whitespace-pre",
              l.kind === "add" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive",
            )}
          >
        ...
        {rest > 0 ? (
          <div className="px-2 py-1 text-[10px] italic text-muted-foreground">… {rest} more changes</div>
        ) : null}
      </div>
    </div>
  );
