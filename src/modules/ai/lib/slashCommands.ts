...
const INIT_PROMPT = `Scan this workspace and produce LABONAIR.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap LABONAIR.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const LABONAIR_CMD_RE =
  /^<labonair-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<labonair-command name="${name}" />\n\n${prompt}`;
}
