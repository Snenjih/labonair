...
export function buildTerminalTools(ctx: ToolContext) {
  return {
    suggest_command: tool({
      ...
      inputSchema: z.object({
        ...
        explanation: z
          .string()
          .optional()
          .describe("Optional one-line note shown alongside in the chat log (not in the terminal)."),
      }),
    ...
    open_preview: tool({
      ...
      inputSchema: z.object({
        url: z.url().describe("Full URL to load (e.g. http://localhost:5173). Must include scheme."),
      }),
      execute: async ({ url }) => {
